...
class PyPreprocess(object):
    ...
    def _run_preprocess(self,  in_filename):
        ...
        c = ['convert',
             ...
             # Removes vertical lines >=60 pixes, reduces widht of >30 (oherwise tesseract < 3.03 completely ignores text close to vertical lines in a table)
             '-negate -define morphology:compose=darken -morphology Thinning Rectangle:1x30+0+0 -negate ',
             '"%s"' % (out_filename)
             ]
    ...
    def preprocess(self, in_filenames):
        ...
        try:
            ...
            preprocessed_filenames = pool.map(
                unwrap_self, list(zip([self]*len(fns), fns)))
            pool.close()
